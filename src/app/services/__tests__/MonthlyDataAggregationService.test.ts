  ...
  describe('calculateMonthlyConsumption', () => {
    ...
    describe('Basic Consumption Calculation', () => {
      it('should calculate correct differences for complete year with actual readings', () => {
        ...
        // Months 2-12 (February-December) should have 100 kWh consumption each
        for (let i = 1; i < 12; i++) {
        ...
        // December (month 12) should have consumption calculated from November
        expect(result[11].month).toBe(12);
        expect(result[11].consumption).toBe(100); // 2200 - 2100
    ...
    describe('December Consumption Using Previous Month', () => {
      it('should calculate December consumption using November (same as other months)', () => {
        ...
        // December should have consumption calculated from November
        expect(result[11].month).toBe(12);
      ...
      it('should handle December with null November gracefully', () => {
        const monthlyData: MonthlyDataPoint[] = [
          ...Array.from({ length: 10 }, (_, i) => createMonthlyPoint(i + 1, 1000 + i * 100, true)),
        ...
        // December should have null consumption (can't calculate with null November)
        expect(result[11].month).toBe(12);
        expect(result[11].consumption).toBeNull();
      });

      it('should handle December with null December reading', () => {
        const monthlyData: MonthlyDataPoint[] = [
          ...Array.from({ length: 11 }, (_, i) => createMonthlyPoint(i + 1, 1000 + i * 100, true)),
        ...
        // December should have null consumption (no December reading)
        expect(result[11].month).toBe(12);
        expect(result[11].consumption).toBeNull();
      });

      it('should work for December even when it is the last month in dataset', () => {
        // This is the key test - December should work without needing next year
        const monthlyData: MonthlyDataPoint[] = Array.from({ length: 12 }, (_, i) =>
          createMonthlyPoint(i + 1, 1000 + i * 100, true, false, false)
        );
        ...
        // December should have consumption (using November from same year)
        expect(result[11].month).toBe(12);
        expect(result[11].consumption).toBe(100); // 2100 - 2000
        expect(result[11].isActual).toBe(true);
      });

      it('should handle previousDecember for January without affecting December', () => {
        const monthlyData: MonthlyDataPoint[] = Array.from({ length: 12 }, (_, i) =>
          createMonthlyPoint(i + 1, 1000 + i * 100, true, false, false)
        );
        ...
        // December should use November from same year
        expect(result[11].month).toBe(12);
        expect(result[11].consumption).toBe(100); // 2100 - 2000
