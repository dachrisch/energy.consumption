...
const Navigation: Component = () => {
  ...
  const [user] = createResource(() => location.pathname, async () => {
    const res = await fetch('/api/session');
    if (!res.ok) return null;
    return res.json();
  });
        ...
        <div class="dropdown dropdown-end">
          ...
          <ul tabindex="0" class="mt-4 z-[100] p-3 shadow-2xl menu menu-sm dropdown-content bg-base-100 rounded-2xl w-64 border border-base-content/10 animate-in fade-in slide-in-from-top-2">
            <div class="px-4 py-4 mb-2 border-b border-base-content/5 bg-base-200/30 rounded-xl">
              <p class="text-sm font-black tracking-tight">{user()?.name || 'Operator'}</p>
              <p class="text-xs opacity-40 font-bold">{user()?.email || 'N/A'}</p>
            </div>
            ...
            <li><button onClick={handleLogout} class="text-error font-black uppercase text-[10px] tracking-widest py-3 hover:bg-error/10">Logout</button></li>
          </ul>
        </div>
      </div>
    </div>
